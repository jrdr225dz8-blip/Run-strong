...
function renderToday(){
 const raw=todayKey();
 const fallback=DAYS[raw]?raw:nextLiftKey();
 const dayKey=selectedDay||fallback;
 const w=DAYS[dayKey];
 const c=state.checkins[sessionKey(dayKey)], status=getStatus(c);
 app.innerHTML=`
 <section class="hero">
  <div class="hero-top">
   <div><span class="badge ${status.cls}">${status.label}</span><div class="title-big">${dayKey===raw?"Today's lift":"Workout preview"}</div><div class="muted">${w.title}</div></div>
   <button id="checkBtn" class="ghost">Body check</button>
  </div>
  <div class="day-selector" aria-label="Choose workout day">
   ${daySelector(dayKey)}
  </div>
  <p class="note day-help">Tap a lifting day to preview the full workout before you get there.</p>
 </section>
 <section class="runna-card">
  <div class="runna-kicker">${dayKey.toUpperCase()} SESSION</div>
  <h2>${w.title}</h2>
 ...
 document.querySelectorAll(".select-day").forEach(btn=>btn.onclick=()=>{
   if(btn.dataset.key && DAYS[btn.dataset.key]){selectedDay=btn.dataset.key;renderToday();}
 });
 wireVideoButtons();
}
function daySelector(active){
 const items=[
  ["monday","MON","Lift"],["tuesday","TUE","Hard"],["wednesday","WED","Lift"],
  ["thursday","THU","Easy"],["friday","FRI","Hard"],["saturday","SAT","Lift"],["sunday","SUN","Long"]
 ];
 return items.map(([key,label,type])=>{
   const lift=!!DAYS[key];
   return `<button class="select-day ${active===key?"active":""} ${lift?"has-lift":"run-only"}" ${lift?`data-key="${key}"`:"disabled"}>
...
function exerciseRow(x,label){
 return `<div class="exercise-row">
  <div class="exercise-index">${label}</div>
  <div class="exercise-copy"><strong>${x.name}</strong><small>${x.sets} × ${x.min}–${x.max} ${x.unit} • ${x.meta}</small></div>
  <button class="video-btn" data-q="${encodeURIComponent(x.videoQ)}">▶ Video</button>
  <div class="mini-picture">${movementPicture(x.type,x.name)}</div>
 </div>`;
}
...
function renderGuided(){
 ...
 <section class="focus-card">
  ...
  <div class="picture-panel">
   <div class="picture-title">Movement pictures</div>
   ${movementPicture(step.ex.type,step.ex.name,true)}
   <p class="picture-caption">Use the pictures for the movement pattern; use the video above when you want to see the full exercise performed.</p>
  </div>
...
function movementPicture(type,name,large=false){
 const accent={quad:"#59a7ff",hinge:"#9b7cff",push:"#ff9a52",pull:"#6edc88",calf:"#f4ca57",abs:"#ff7575"}[type]||"#59a7ff";
 const pose={
  quad:{a:[160,75,138,112,120,145,182,112,201,145],b:[160,78,132,106,105,137,186,109,214,137]},
  hinge:{a:[160,72,146,108,127,143,178,108,197,143],b:[151,78,118,103,94,128,170,110,195,143]},
  push:{a:[160,72,128,92,100,92,192,92,220,92],b:[160,72,138,103,113,119,182,103,207,119]},
  pull:{a:[160,72,130,98,106,119,190,98,214,119],b:[160,72,142,92,129,73,178,92,191,73]},
  calf:{a:[160,72,145,109,139,145,175,109,181,145],b:[160,72,145,108,143,139,175,108,177,139]},
  abs:{a:[160,72,140,104,124,136,180,104,196,136],b:[150,82,126,103,111,127,171,104,191,129]}
 }[type]||{a:[160,72,140,105,125,140,180,105,195,140],b:[160,72,135,100,115,132,185,100,205,132]};
 function frame(x,arr,title){
  return `<g transform="translate(${x},0)">
   <rect x="0" y="0" width="145" height="125" rx="16" fill="#0f151d" stroke="#2c3847"/>
   <circle cx="${arr[0]-88}" cy="${arr[1]-28}" r="12" fill="#f5f7fb"/>
   <path d="M${arr[0]-88} ${arr[1]-15} L${arr[0]-88} ${arr[3]-20}" stroke="#f5f7fb" stroke-width="8" stroke-linecap="round"/>
   <path d="M${arr[0]-88} ${arr[1]-4} L${arr[2]-88} ${arr[3]-12}" stroke="${accent}" stroke-width="8" stroke-linecap="round"/>
   <path d="M${arr[0]-88} ${arr[1]-4} L${arr[6]-88} ${arr[7]-12}" stroke="${accent}" stroke-width="8" stroke-linecap="round"/>
   <path d="M${arr[0]-88} ${arr[3]-20} L${arr[4]-88} ${arr[5]-16}" stroke="${accent}" stroke-width="9" stroke-linecap="round"/>
   <path d="M${arr[0]-88} ${arr[3]-20} L${arr[8]-88} ${arr[9]-16}" stroke="${accent}" stroke-width="9" stroke-linecap="round"/>
   <text x="72" y="116" fill="#91a2b7" text-anchor="middle" font-size="10">${title}</text>
  </g>`;
 }
 return `<svg class="${large?"movement-svg large":"movement-svg"}" viewBox="0 0 310 135" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="${name} start and finish positions">
  ${frame(5,pose.a,"START")}${frame(160,pose.b,"FINISH")}
  <path d="M145 62 H160" stroke="${accent}" stroke-width="4" stroke-linecap="round"/><path d="M156 56 L163 62 L156 68" fill="none" stroke="${accent}" stroke-width="3" stroke-linecap="round"/>
 </svg>`;
}
