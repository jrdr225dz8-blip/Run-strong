...
function blocksFor(id,w){
 ...
 if(!blocks) return [{label:"A",kind:"solo",rest:90,ids:w.xs.map(x=>x.id)}];
 const byId=Object.fromEntries(w.xs.map(x=>[x.id,x]));
 return blocks.map(b=>({...b,items:b.ids.map(i=>byId[i]).filter(Boolean)})).filter(b=>b.items.length);
}
...
function buildGuidedSteps(id,w){
 ...
 blocks.forEach(b=>{
  if(b.kind==="warmup"){
   b.items.forEach((x,idx)=>steps.push({x,block:b.label,kind:b.kind,set:1,totalSets:1,rest:0,lastInRound:idx===b.items.length-1}));
   return;
  }
...
function renderGuided(){
 ...
 <div class="next">${nextInstruction()}</div>
 ...
 doneG.onclick=()=>{logStep(st);if(st.rest>0)startRest(st.rest);else{guided.i++;renderGuided()}};
}
